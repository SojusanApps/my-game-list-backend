...
- **Task Runner:** Utilize the project's `Makefile`. Run `make help` to see available commands.
...
- **Validation:** Always validate changes with `make check` before concluding your work.

## Testing & Coverage

- **Framework:** Use `pytest` via `make test` or `tox`. Target test coverage must remain >90%.
- **Dependencies:** Tests and local execution require an active PostgreSQL instance.
  - To test: Start the test DB with `make test_db` beforehand.
  - To run app locally: Start the app DB with `make app_db` first, then `make fresh_run` or `make run`.
